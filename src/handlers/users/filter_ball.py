...
async def inline_search_ball(inline_query: InlineQuery, state: FSMContext):
    ...
    if shakl == "gr":
        ...
        if text:
            cursor.execute("SELECT r.id, r.region_name FROM mandat m JOIN regions r ON m.region_id = r.region_id WHERE lower(un_text) LIKE ? and m.con_b <= ? AND m.con_b != 0", (f"%{text}%", ball))
        else:
            cursor.execute(
...
async def inline_search_university(inline_query: InlineQuery, state: FSMContext):
    ...
    if shakl == "gr":
        ...
        if text:
            cursor.execute(
                "SELECT u.id, u.un_text FROM universities u JOIN mandat m ON u.un_id = m.un_id AND u.region_id = m.region_id "
                "WHERE lower(un_text) LIKE ? and m.region_id = ? AND m.con_b <= ? AND m.con_b != 0", (f"%{text}%", reg_id, ball))
        else:
            cursor.execute(
                "SELECT u.id, u.un_text FROM universities u JOIN mandat m ON u.un_id = m.un_id AND u.region_id = m.region_id "
...
async def chosen_university(message: Message, state: FSMContext):
    if message.text == "🔙 Ortga":
        ...
        ball = data["ball"]
        if shakl == "gr":
            rows = cursor.execute("SELECT g.ty_text FROM gettypes g JOIN mandat m ON g.un_id = m.un_id AND g.region_id = m.region_id "
                                  "WHERE m.region_id = ? AND m.un_id = ? AND m.gr_b <= ? AND m.gr_b != 0  AND m.ty_id = 1", (reg_id, un_id, ball))
            keyboard = []
