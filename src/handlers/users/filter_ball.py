...
def _ball_condition(shakl: str) -> str:
    """SQL WHERE sharti."""
    if shakl == "gr":
        return "m.gr_b <= %s AND m.gr_b != 0 AND m.ty_id = '1'"
    return "m.con_b <= %s AND m.con_b != 0"
...
async def back_from_ball1(message: Message, state: FSMContext):
...
async def enter_ball(message: Message, state: FSMContext):
    ...
    if not message.text.isdigit():
        await message.answer("<b>Ball xato kiritildi! Faqat son kiriting:</b>",
                             parse_mode="html", reply_markup=await UserPanels.to_back())
        return

    ball = int(message.text)
    if ball > 200:
        await message.answer("<b>Ball 200 dan oshmasligi kerak!</b>",
                             parse_mode="html", reply_markup=await UserPanels.to_back())
        return
    ...
    with db_connection() as (conn, cur):
        cur.execute(f"""
            SELECT DISTINCT r.region_name FROM mandat m
    ...
    if not regions:
        await message.answer("<b>🤷🏻‍♂️ Bu ball bilan hech qaysi yo'nalish topilmadi</b>",
                             parse_mode="html")
...
# ═══════════════════════════════════════════════════════════════
#  BALL2 — Hudud inline qidiruv + tanlash
# ═══════════════════════════════════════════════════════════════
...
async def inline_ball2(inline_query: InlineQuery, state: FSMContext):
    ...
    with db_connection() as (conn, cur):
        if text:
            cur.execute(f"""
                SELECT DISTINCT r.id, r.region_name FROM mandat m
                JOIN regions r ON m.region_id = r.region_id
...
async def chosen_region(message: Message, state: FSMContext):
    ...
    with db_connection() as (conn, cur):
        cur.execute("SELECT region_id FROM regions WHERE lower(region_name) = %s", (reg_name.lower(),))
        row = cur.fetchone()
        if not row:
            return
        reg_id = row[0]
...
# ═══════════════════════════════════════════════════════════════
#  BALL3 — Universitet inline + tanlash
# ═══════════════════════════════════════════════════════════════
...
async def inline_ball3(inline_query: InlineQuery, state: FSMContext):
    ...
    with db_connection() as (conn, cur):
        if text:
            cur.execute(f"""
                SELECT DISTINCT u.id, u.un_text FROM universities u
                JOIN mandat m ON u.un_id = m.un_id AND u.region_id = m.region_id
                WHERE lower(u.un_text) LIKE %s AND m.region_id = %s AND {cond}
            """, (f"%{text}%", reg_id, ball))
        else:
            cur.execute(f"""
                SELECT DISTINCT u.id, u.un_text FROM universities u
...
async def chosen_university(message: Message, state: FSMContext):
    ...
    with db_connection() as (conn, cur):
        if shakl == "gr":
            ...
            cur.execute(f"""
                SELECT DISTINCT g.ty_text FROM gettypes g
                JOIN mandat m ON CAST(g.un_id AS VARCHAR) = m.un_id AND g.region_id = m.region_id
                WHERE m.region_id = %s AND m.un_id = %s AND {cond}
            """, (reg_id, str(un_id), ball))
            types = cur.fetchall()

    keyboard = [[KeyboardButton(text=ty_text if isinstance(ty_text, str) else ty_text[0])]
                for ty_text in types]
    keyboard.append([KeyboardButton(text="🔙 Ortga"), KeyboardButton(text="🔙 Bosh menu")])
    btn = ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True)
...
async def chosen_type(message: Message, state: FSMContext):
    if message.text == "🔙 Ortga":
        ...
        await message.answer(f"<b>{count} ta oliygoh\n\n🏢 OTMni tanlang:</b>",
                             parse_mode="html", reply_markup=await UserPanels.to_back())
        await message.answer("<b>Tezkor qidiruvdan foydalaning... 👇</b>", parse_mode="html", reply_markup=kb)
        return
    ...
    with db_connection() as (conn, cur):
        ...
        row = cur.fetchone()
        if not row:
            # Grand uchun hardcoded "Kunduzgi" → ty_id=1
            if name == "kunduzgi":
                ty_id = "1"
            else:
                return
    ...
    with db_connection() as (conn, cur):
        cur.execute(f"""
            ...
            WHERE m.un_id = %s AND m.ty_id = %s AND m.region_id = %s AND {cond}
...
async def chosen_lang(message: Message, state: FSMContext):
    if message.text == "🔙 Ortga":
        ...
        with db_connection() as (conn, cur):
            if shakl == "gr":
                ...
                """, (reg_id, str(un_id), ball))
                types = cur.fetchall()
        keyboard = [[KeyboardButton(text=t[0])] for t in types]
        keyboard.append([KeyboardButton(text="🔙 Ortga"), KeyboardButton(text="🔙 Bosh menu")])
        btn = ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True)
        await message.answer("<b>🔰 Ta'lim shaklini tanlang: 👇</b>", parse_mode="html", reply_markup=btn)
        return
    ...
    with db_connection() as (conn, cur):
        cur.execute(f"""
            ...
            WHERE region_id = %s AND un_id = %s AND ty_id = %s AND lan_id = %s AND {cond}
...
async def chosen_direction(message: Message, state: FSMContext):
    ...
    with db_connection() as (conn, cur):
        cur.execute(f"""
            ...
            AND mvdir = %s AND nomi = %s AND region_id = %s AND {cond}
    ...
    if create_card(univer=un_name, faculty=f"{mvdir} - {nomi}", lang=lan_text, edu=ty_text,
        ...
        try:
            sent    = await message.answer_photo(photo=FSInputFile(path), caption=msg_text, parse_mode="html")
